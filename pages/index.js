      ...
      <div className="max-w-xl mx-auto px-6 py-20">
        <header className="mb-16">
          ...
          <p className="text-slate-400 leading-relaxed text-sm max-w-md">
            ...
            <span className="text-slate-600 italic text-xs block mt-2">Recording life between simulations. No data, just fragments.</span>
          </p>
        </header>

        <section className="space-y-12">
          <h2 className="text-[10px] uppercase tracking-[0.5em] text-emerald-500/40 font-bold border-b border-slate-900 pb-2">Timeline</h2>
          {posts.length === 0 ? (
            <p className="text-slate-700 text-xs italic font-mono">No public logs found. Check Notion Status.</p>
          ) : (
            posts.map((post) => (
              <div key={post.id} className="group relative border-l border-slate-800 pl-6 py-1 hover:border-emerald-500/50 transition-all">
                <div className="absolute -left-[3px] top-2 w-1.5 h-1.5 rounded-full bg-slate-800 group-hover:bg-emerald-500" />
                <div className="mb-2 flex flex-wrap items-center gap-3 text-[10px] font-mono text-slate-500">
                  <span className="text-slate-400">{post.date}</span>
                  {post.place && <span className="text-slate-700">@ {post.place}</span>}
                  {post.mood && <span className="text-emerald-500/60"># {post.mood}</span>}
                </div>
                <h3 className="text-slate-100 font-medium text-lg group-hover:text-white transition-colors">{post.title}</h3>
              </div>
            ))
          )}
        </section>
