...
export default function Home({ posts }) {
  return (
    <div className="min-h-screen bg-[#050505] text-slate-200 font-sans selection:bg-emerald-500/30">
      <Head>
        <title>Eric T. | Digital Space</title>
      </Head>
      ...
      <div className="max-w-2xl mx-auto px-6 py-20">
          ...
          {posts.length === 0 ? (
            <div className="py-10 text-center">
              <p className="text-slate-700 text-xs italic font-mono">
                ...
                <span className="text-[10px] opacity-40">(Ensure Notion "Status" is set to "Done")</span>
              </p>
            </div>
          ) : (
            <div className="space-y-10">
              {posts.map((post) => (
                <div key={post.id} className="group relative border-l border-slate-800 pl-6 py-1 hover:border-emerald-500/50 transition-all">
                  <div className="absolute -left-[4px] top-2.5 w-2 h-2 rounded-full bg-slate-800 border border-[#050505] group-hover:bg-emerald-500 transition-all shadow-[0_0_8px_rgba(16,185,129,0.3)]" />
        ...
        {/* --- Footer Section --- */}
        <footer className="mt-40 pt-8 border-t border-slate-900 opacity-20 text-[9px] font-mono tracking-[0.4em] text-center">
